...
- **Node.js version**: 22.x or newer (see `engines.node` in `package.json`;
...
4. Deploy. **The build succeeds even with zero environment variables set**
   — public marketing/auth/legal/help/blog pages are statically generated
   and render normally either way — but a real deployment needs at least
   the Supabase variables (below) for anything behind login to work.
...
| `RESEND_API_KEY`, `EMAIL_FROM` | Transactional emails (welcome, payment failed, contact form, etc.) | Email sends throw a clear server-side error; the rest of the app is unaffected |
...
| `TESTING_MODE`, `EMAIL_TEST_OVERRIDE_TO`, `ANALYTICS_HASH_SALT`, `GROQ_WHISPER_MODEL` | Dev/staging conveniences | Never set `TESTING_MODE` in production — it bypasses all credit/plan gating |
...
Vercel does not run database migrations for you. Apply every file under
