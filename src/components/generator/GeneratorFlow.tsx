        ...
        <div className="reveal-in flex min-h-0 flex-1 flex-col gap-3 overflow-hidden">
          <div className="flex flex-wrap items-center justify-between gap-2 rounded-2xl border border-white/[0.07] bg-[#0b0f1c] px-3 py-2.5 shadow-lg">
            <div className="flex items-center gap-1">
              <button onClick={undo} disabled={!history.length} className="focus-ring rounded-lg p-2 text-white/35 hover:bg-white/[0.05] disabled:opacity-25" title="Undo" aria-label="Undo"><Undo2 size={15} /></button>
              <button onClick={redo} disabled={!future.length} className="focus-ring rounded-lg p-2 text-white/35 hover:bg-white/[0.05] disabled:opacity-25" title="Redo" aria-label="Redo"><Redo2 size={15} /></button>
              <span className="mx-1 h-5 w-px bg-ink/10" />
              <button onClick={() => setMode("visual")} className={`focus-ring flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs ${mode === "visual" ? "bg-signal text-white" : "text-white/40 hover:bg-white/[0.05]"}`}><Eye size={13} /> Visual</button>
              <button onClick={() => setMode("code")} className={`focus-ring flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs ${mode === "code" ? "bg-signal text-white" : "text-white/40 hover:bg-white/[0.05]"}`}><Code2 size={13} /> Code</button>
            </div>
