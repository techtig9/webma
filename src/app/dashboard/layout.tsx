...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="flex min-h-screen bg-[#070a12] text-white">
    ...
    <div className="min-w-0 flex-1">
      <Topnav name={profile?.name ?? user.email ?? "User"} email={user.email ?? ""} plan={profile?.role === "admin" ? "admin" : subscription?.plan ?? "free"} creditsRemaining={profile?.role === "admin" ? Infinity : subscription?.credits_remaining ?? 0} isAdmin={profile?.role === "admin"} />
