...
export default async function DashboardPage() {
  ...
  return <div className="space-y-7">
    <section className="relative overflow-hidden rounded-3xl border border-white/[0.08] bg-gradient-to-br from-signal/[0.16] via-[#11162a] to-[#0b0f1c] p-6 lg:p-8">
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <Stat label="AI credits remaining" value={subscription?.credits_remaining ?? 0} icon={Sparkles} accent="signal" />
    </section>
    ...
    <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
      ...
      <div className="space-y-5">
        ...
        <div className="saas-card p-5"><div className="flex items-center justify-between"><span className="text-xs text-white/40">Current plan</span><span className="rounded-full bg-signal/10 px-2 py-1 text-[10px] capitalize text-signal">{subscription?.plan ?? "free"}</span></div><div className="mt-4 h-2 overflow-hidden rounded-full bg-white/[0.06]"><div className="h-full w-1/3 rounded-full bg-signal"/></div><p className="mt-2 text-[10px] text-white/30">{subscription?.credits_remaining ?? 0} AI credits remaining</p><Link href="/dashboard/billing" className="mt-4 block text-xs font-semibold text-signal hover:text-white">Manage plan →</Link></div>
      </div>
    </section>
  </div>;
}
